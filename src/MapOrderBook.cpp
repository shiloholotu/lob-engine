...
void MapOrderBook::add(const Order& order) {
...
Order* MapOrderBook::bestBidFront() {
    if (!bids_.empty()){
...
Order* MapOrderBook::bestAskFront() {
    if (!asks_.empty())
        return &(asks_.begin()->second.front());
    return nullptr;
}

const Order* MapOrderBook::bestBidFront() const {
    if (!bids_.empty())
        return &(bids_.begin()->second.front());
    return nullptr;
}

const Order* MapOrderBook::bestAskFront() const {
    if (!asks_.empty())
        return &(asks_.begin()->second.front());
    return nullptr;
}
